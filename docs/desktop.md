...
- **Mac, Apple Silicon:** Extract `KiCad-Component-Packager-0.1.12-macOS-AppleSilicon.zip` and open `KiCad Component Packager.app`. You can move the app to Applications. Tested on macOS 15.3.1; Intel Macs use the separate macOS-Intel release ZIP.
- **Windows, x64:** Extract **the entire** `KiCad-Component-Packager-0.1.12-Windows-x64.zip`, then open `KiCad Component Packager.exe` inside its folder. Keep the adjacent files and `resources` folder together. Python and Node do not need to be installed.
